...
def load_alpaca_ohlcv(
    ...
    cache_dir: Optional[Union[str, Path]] = "data",
    ...
    cache_dir : optional path
        ...
        <cache_dir>/alpaca_<SYMBOL>_<rule>.csv and reused on later runs: if it
...
timestamp_is_bar_open : optional bool
    ...
    if cache_dir is not None:
        cache_path = Path(cache_dir) / f"alpaca_{symbol.replace('/', '_')}_{pandas_rule}.csv"
    ...
    if cache_path is not None:
        _write_cached_csv(df, cache_path)
...
def _read_cached_csv(path: Path) -> Optional[pd.DataFrame]:
    try:
        df = pd.read_csv(path)
        idx = pd.to_datetime(df["Time"], errors="coerce", utc=True)
        df = df.loc[~idx.isna()].copy()
        df.index = pd.DatetimeIndex(idx.loc[~idx.isna()], name="Time")
        return df[OHLCV].sort_index()
    except Exception:
        return None


def _write_cached_csv(df: pd.DataFrame, path: Path) -> None:
    path = Path(path)
