...
class BracketTradingEnvV2(gym.Env):
    ...
    def _observation(self) -> np.ndarray:
        market = self.episode_features.iloc[self.index].to_numpy(dtype=np.float32)
        position = self.core.position
        state = np.array([position.direction, position.decision_bars / 24, self.core.equity / self.core.initial_equity - 1], dtype=np.float32)
