...
class CausalRegimeModel:
    ...
    def forward_probabilities(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        emissions = np.full((len(values), 3), np.nan, dtype=float)
        if valid.any():
            for component, (mean, covariance) in enumerate(zip(self.model.means_, self.model.covars_)):
                emissions[valid, component] = multivariate_normal.logpdf(
                    values[valid], mean=mean, cov=covariance, allow_singular=True
                )
        ...
        for row_index, emission in enumerate(emissions):
            ...
            output[row_index] = np.exp(alpha)[order]
