...
def fracdiff_weights(d: float, threshold: float = 1e-5, max_size: int = 100_000) -> np.ndarray:
    """Return causal fixed-width binomial weights ordered current-to-oldest."""
    if not 0 <= d <= 1:
...
def fixed_width_fracdiff(series: pd.Series, d: float, threshold: float = 1e-5) -> pd.Series:
    ...
    if invalid.any():
        contaminated = np.convolve(invalid.astype(int), np.ones(len(weights), dtype=int), mode="full")[: len(values)] > 0
