...
- Authorized chats may use status-message buttons or `/progress`, `/status`, `/help`, and `/clear` commands to inspect progress and clear messages tracked during the current run.
...
- PuffeRL uses an 11-level categorical residual policy around the causal alpha target so PPO log-probabilities match sampled training actions; deterministic validation and live inference use the probability-weighted ordered residual, and the zero-residual checkpoint is the validation incumbent.
